...
for f in range(epoch):
	...
	if f%20000 == 19999:
		...
		print("---------")
		probs = sess.run(logits, feed_dict={inputXY: [[key_to_int[1208], key_to_int[1210]]]})
		print(int_to_key[np.argmax(probs)])
		probs = sess.run(logits, feed_dict={inputXY: [[key_to_int[2050], key_to_int[1210]]]})
		print(int_to_key[np.argmax(probs)])
		probs = sess.run(logits, feed_dict={inputXY: [[key_to_int[2956], key_to_int[1210]]]})
		print(int_to_key[np.argmax(probs)])
		probs = sess.run(logits, feed_dict={inputXY: [[key_to_int[2598], key_to_int[1210]]]})
		print(int_to_key[np.argmax(probs)])
print(avg_err)
